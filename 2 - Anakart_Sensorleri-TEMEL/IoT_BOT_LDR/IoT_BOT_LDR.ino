...
int LDRPini = 39;                                     // LDR sensörümüzün bağlı olduğu pini tanımladık.
int LDRDegeri;                                        // LDR sensörümüzden okunan değerleri integer değer atadık.
...
void setup()
{
  ...
  Serial.println("LDR Işık Testi...");
  
  pinMode(LDRPini, INPUT);                            // LDR sensörümüzü veri aldığı için giriş olarak belirledik.
}

//---------------------------------------   // SÜREKLİ ÇALIŞACAK DÖNGÜ İÇİNDEKİ KISIM   ----------------------------------------------------
void loop()
{
  LDRDegeri = analogRead(LDRPini);                    // Sensörden sürekli analog olarak değer okuma komutu veriyoruz.
  
  Serial.print("Okunan Değer: ");                     // Seri ekrana bilgilendirme mesajı gönderdik.
  Serial.println(LDRDegeri);                          // Okuduğumuz değeri seri ekrana bilgilendirme mesajı olarak gönderdik.
  
  lcd.setCursor(2, 3);                                // İmleci, yazmak istediğimiz koordinata yönlendiriyoruz.
  lcd.print("LDR Degeri: ");                          // Ekranda yazmasını istediğimiz yazıları yolluyoruz.
  lcd.print(LDRDegeri);                               // Ekranda yazmasını istediğimiz yazıları yolluyoruz.
...
void yan_son_yazi()                                     // Yanıp sönen bilgilendirme mesajını hazırlıyoruz.
{
  ...
  if (i < 15)
  {
    ...
    lcd.print("Isigi Degistiriniz");                    // Ekranda yazmasını istediğimiz yazıları yolluyoruz.
  }
  if (i >= 15)
  {
    lcd.setCursor(1, 2);                                // İmleci, yazmak istediğimiz koordinata yönlendiriyoruz.
